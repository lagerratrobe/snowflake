...
-- 10.0.0  Determine Appropriate Warehouse Sizes

-- 10.1.0  Run a sample query with an extra small warehouse
...
-- 10.1.2  Create a new worksheet named Warehouse Sizing with the following
--         context:
...
ALTER WAREHOUSE VIPER_WH
SET WAREHOUSE_SIZE = XSmall;


-- 10.1.4  Suspend and resume the warehouse to make sure its Data Cache is
--         empty, disable the query result cache and set a query tag:

ALTER WAREHOUSE VIPER_WH SUSPEND;
ALTER WAREHOUSE VIPER_WH RESUME;
ALTER SESSION SET USE_CACHED_RESULT = FALSE;
ALTER SESSION SET QUERY_TAG = 'VIPER_WH_Sizing';


-- 10.1.5  Run the following query (your test query - it will be used throughout
...
-- 10.1.7  Take note of the performance metrics for this operator: you should
...
-- 10.2.0  Run a sample query with a medium warehouse

-- 10.2.1  Suspend your warehouse, change its size to Medium, and resume it:

ALTER WAREHOUSE VIPER_WH SUSPEND;
ALTER WAREHOUSE VIPER_WH SET WAREHOUSE_SIZE = Medium;
ALTER WAREHOUSE VIPER_WH RESUME;
...
-- 10.2.4  Take note of the performance metrics for this operator. You should
--         see lower amounts spilling to local storage and remote disk, as well
--         as faster execution.

-- 10.3.0  Run a sample query with a large warehouse

-- 10.3.1  Suspend your warehouse, change its size to Large, and resume it:

ALTER WAREHOUSE VIPER_WH SUSPEND;
ALTER WAREHOUSE VIPER_WH SET WAREHOUSE_SIZE = Large;
ALTER WAREHOUSE VIPER_WH RESUME;
...
-- 10.3.3  Take note of the performance metrics for this operator. You will see
--         that there is no spilling to local or remote storage.

-- 10.4.0  Run a sample query with an extra large warehouse

-- 10.4.1  Suspend the warehouse, change it to XLarge in size, and resume it:

ALTER WAREHOUSE VIPER_WH SUSPEND;
ALTER WAREHOUSE VIPER_WH SET WAREHOUSE_SIZE = XLarge;
ALTER WAREHOUSE VIPER_WH RESUME;
...
-- 10.5.0  Run a query showing the query history results for these performance
...
WHERE  query_tag = 'VIPER_WH_Sizing' 
AND WAREHOUSE_SIZE IS NOT NULL
AND QUERY_TYPE LIKE 'SELECT' ORDER BY start_time DESC;


-- 10.6.0  This query will show more information but the ACCOUNT_USAGE schema
