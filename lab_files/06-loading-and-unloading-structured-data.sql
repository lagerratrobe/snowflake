...
-- 6.0.0   Loading and Unloading Structured Data
--         Expect this lab to take approximately 30 minutes.

-- 6.1.0   Create Tables and File Formats
...
-- 6.1.1   Navigate to [Worksheets] and create a new worksheet named Load
...
-- 6.1.3   Create a file format called MYPIPEFORMAT, that will read the pipe-
...
-- 6.1.4   Create a file format called MYGZIPPIPEFORMAT that will read the
...
-- 6.3.0   Load a GZip Compressed File
...
-- 6.3.1   Empty the REGION Table in the PUBLIC schema of VIPER_DB:
...
-- 6.4.0   Load data using the Load Data Wizard
...
-- 6.4.1   First, run the following query to generate the file you’re going to
...
-- 6.4.2   You should see five rows in the results. Click the download button to
--         download the file:
--         Download Button

-- 6.4.3   You will see a dialog box offering you either TSV or CSV format.
--         Select CSV and click Export.

-- 6.4.4   Your file should have saved to the location you selected. Examine the
--         file using the text editor of your choice. The file should have a
--         header row and five rows of data.

-- 6.4.5   Now let’s create a pipe format using the user interface. In the Web
--         UI, click the Databases button in the ribbon at the top, select your
--         database, then click the File Formats tab.

-- 6.4.6   You should see a list of existing file formats, including the ones
--         you created. Click Create.

-- 6.4.7   The Create File Format dialog box should appear. Type MYCSVFORMAT in
...
-- 6.4.8   Your new file format should now be listed. Now click the Tables tab
--         and select the row for table REGION.

-- 6.4.9   Click the Load Table option just above the table list.

-- 6.4.10  You should now see the Load Data dialog box. You will have to select
--         the warehouse, source files, file format and load options. Select
--         VIPER_WH and click Next.

-- 6.4.11  You should see an option to load a file from your computer. Click the
--         Select Files button, navigate to and select the file you downloaded,
--         then click Next.

-- 6.4.12  You should now see an option to select a file format. Select
--         MYCSVFORMAT and click Next.

-- 6.4.13  You should now see a list of Load Options. These give you different
...
-- 6.4.14  You should see a dialog box saying files are being encrypted and
...
-- 6.4.15  The error should read Numeric value R_REGIONKEY is not recognized.
...
-- 6.5.0   Unload a Pipe-Delimited File to a Table Stage
...
-- 6.5.3   Unload the data to the REGION table stage. Remember that a table
...
-- 6.5.5   OPTIONAL: Use GET to download the file to your local system. Open it
...
-- 6.6.5   OPTIONAL: Use GET to download the file to your local system and
