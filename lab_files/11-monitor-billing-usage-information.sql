...
-- 11.0.0  Monitor Billing & Usage Information
...
-- 11.1.0  Review Warehouse Usage

-- 11.1.1  Navigate to [Warehouses].

-- 11.1.2  Select VIPER_WH.

-- 11.1.3  Review the Warehouse Load Over Time information.

-- 11.1.4  Navigate to [Account].

-- 11.1.5  Make sure the Billing & Usage section is selected. Review the
...
-- 11.1.7  Use SQL and the Information Schema WAREHOUSE_METERING_HISTORY table
...
-- 11.1.8  Starting with the query above, write your own query to look at the
...
-- 11.2.0  Review Billing & Usage Information
...
-- 11.2.5  Review the number of Warehouses and total credits billed and take
--         note of which month the activity is listed.
--         In the detailed activity, identify:

-- 11.2.6  Navigate to [Warehouses].

-- 11.2.7  Locate and click on your VIPER_WH and evaluate your usage over
--         time.

-- 11.2.8  Expand the time frame of the bottom graph and see what happens to the
--         top graph.
...
-- 11.2.12 Review the Daily Average and the Rolling Monthly Average Line on the
--         graph.
...
-- 11.2.15 Execute the following commands to pull various Billing & Usage
--         metrics:

-- 11.2.16 Pull all Warehouse usage for the last rolling seven (7) days from the
--         Information Schema:
...
-- 11.3.0  View and Set Parameters
...
-- 11.3.2  Pull the value for the DATE_OUTPUT_FORMAT parameter for your session.
--         The format set should equal the default format of YYYY-MM-DD:

SHOW PARAMETERS LIKE 'DATE_OUTPUT_FORMAT' IN SESSION;


-- 11.3.3  Run a query to SELECT the current date, and notice the format of the
--         date returned:
...
-- 11.3.5  Re-run the query to SELECT the current date. Notice the format of the
...
-- 11.4.0  Evaluate Account and Object Information
...
-- 11.5.4  Toggle between the Database, Stage, and Fail Safe categories and
--         review the change in storage for each throughout the month.

-- 11.5.5  Select the Worksheets tab. You may use the same context from the
--         previous task.

-- 11.5.6  Run a query to return average daily storage usage for the past 5
...
-- 11.5.7  Modify the query to return average daily storage usage for the past 5
...
-- 11.5.8  Run a query to return average daily data storage usage for all the
...
-- 11.5.9  Pull total storage bytes for the last rolling seven (7) days from
--         Account Usage.
...
-- 11.5.10 Pull storage for Internal Stages for the previous 5 days. In the
...
-- 11.6.0  Query Views in Information Schema

-- 11.6.1  Open a new Worksheet. Re-name the worksheet to Information Schema.

-- 11.6.2  Retrieve the hourly Warehouse usage for the current month for your
--         VIPER_WH Virtual Warehouse, ordered by time:

SELECT *
FROM TABLE(information_schema.warehouse_metering_history
(date_range_start=>date_trunc(month, current_date),
date_range_end=>dateadd(month,1,date_trunc(month, current_date)),'VIPER_WH'))
ORDER BY start_time;


-- 11.6.3  Retrieve average daily storage for the past three (3) days, for
--         VIPER_DB Database:
...
-- 11.6.4  Directly query the Information Schema TABLES View to find the size of
...
-- 11.6.5  Join the APPLICABLE_ROLES and DATABASES Information Schema Views to
...
WHERE r.grantee = 'VIPER';


-- 11.7.0  Query Views in Account Usage

-- 11.7.1  Retrieve the number of failed logins, by user, month-to-date from the
...
-- 11.7.2  Determine the busiest Warehouse by number of jobs executed, by
...
-- 11.7.3  Determine the number of used credits and costs by all Warehouses
...
-- 11.7.4  Determine the amount of storage and associated costs used within an
...
-- 11.7.6  Show the top 10 users in terms of total execution time within the
