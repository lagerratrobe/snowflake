...
-- 3.1.4   You should now see the Worksheets pane. Click the […] button in the
--         upper right hand corner.

-- 3.1.5   A drop down menu should appear. Choose the Import Worksheets option.

-- 3.1.6   You will see an Import Worksheets dialog box. Click the Import button
--         in the dialog box.

-- 3.2.0   Working with worksheets.
...
-- 3.2.3   Click on the worksheet name drop down arrow and click on the
--         worksheet name.
--         Renaming the Worksheet

-- 3.2.4   Rename the worksheet to Member List and hit Enter.

-- 3.3.0   Setting the context
...
-- 3.3.3   In the upper left-hand corner just above the SQL pane, change the
...
-- 3.3.5   Scroll using the horizontal and vertical scroll bars in order to
--         examine all of the available fields.

-- 3.3.6   Click outside the preview window to close the preview window.

-- 3.4.0   Write and run a query

-- 3.4.1   Click back to the query screen.

-- 3.4.2   SLOWLY TYPE the following query into the query screen. Do not cut and
--         paste. As hints pop up, press the Tab key to auto-complete:

SELECT * FROM MODELED.MEMBERS M;


-- 3.4.3   Run the query by pressing the run button in the upper right-hand
...
-- 3.4.6   Now let’s add the first name and last name columns. Type a comma
...
-- 3.4.7   Now let’s make the query easier to read. Click on the drop down arrow
...
-- 3.5.0   Working with worksheet history
...
-- 3.5.1   Modify the query to include the city and state columns and run the
--         query again.

-- 3.5.2   Modify the query again to include the points_balance column and run
--         the query again.

-- 3.5.3   Now that we have a worksheet history to examime, click on the drop
--         down arrow below the blue run arrow.
--         Note that there are several time slots now.

-- 3.5.4   For any of the time slot entries, float your arrow to the right of
--         the entry to see the preview link.

-- 3.5.5   Next, float your arrow over the link to see a preview of the SQL
...
-- 3.6.0   Change the sort order of a column.
...
-- 3.7.0   Create a second Worksheet with another query
...
-- 3.7.2   From the home screen, click the blue + Worksheet button in the upper
--         right hand corner.
--         A new empty worksheet should be displayed.

-- 3.7.3   Rename the worksheet to Points for Members and add in the following
--         query:
...
-- 3.7.4   Click the drop down arrow next to the worksheet name and select
--         Format query.

-- 3.7.5   Click the run button. (Arrow in the top right corner of the screen)

-- 3.8.0   Using the Automatic Contextual Statistics section
...
-- 3.8.1   Locate the filters pane to the right of the result pane and click the
...
-- 3.8.3   Click on each column header and note how the column you clicked is
...
-- 3.9.0   Creating a folder and adding worksheets into the folder
...
-- 3.9.2   Click the ellipsis (…) next to the +Worksheet button and select New
...
-- 3.9.4   Click Worksheets above Member Queries to return to the root of the
--         Worksheets pane.
...
-- 3.9.7   You should now see the contents of the Members List worksheet, to
--         include the SQL code and perhaps the results.

-- 3.9.8   Click the drop down arrow next to the worksheet name and select Move
--         to and the Members Queries folder.
--         Move To Members Queries

-- 3.9.9   Repeat the steps to move the Points for Members query into the
--         Members Queries folder.

-- 3.9.10  Click the Members Queries folder link in the bread crumb trail to
--         check if the two queries are in the folder.
--         Checking the Members Queries folder

-- 3.10.0  Sharing a worksheet
...
-- 3.10.3  Using the skills you’ve learned, write a simple query. For example,
...
-- 3.10.6  Select another user in the class to share by typing their user name
...
-- 3.11.3  Navigate as shown to choose a custom date range and set the range
...
-- 3.12.0  Working with charts using a GROUP BY Query.
...
-- 3.12.5  Change the look of the chart by clicking on each option in the
...
-- 3.13.0  Working with charts with a query without a GROUP BY.
...
-- 3.13.4  Change the Data from Age to Points_Balance and set the Aggregation to
--         Sum.

-- 3.13.5  Select labels for both axes.

-- 3.13.6  Hover the cursor over various bars in the bar chart and observe that
...
-- 3.14.0  Working with a date field as the X-Axis

-- 3.14.1  Change the X-Axis to Started_Date.
--         Notice the X-Axis label doesn’t change and has to be changed
--         manually.

-- 3.15.0  Downloading a chart

-- 3.15.1  Using the month chart from the last section, Click the Download Chart
...
-- 3.16.0  Creating a Dashboard

-- 3.16.1  Using the worksheet with the chart from the last section create a new
...
-- 3.16.3  Locate the Return to Member Charts Link at the top of the page and
--         click it.
--         You should now see the dashboard itself in read only mode.

-- 3.16.4  In the dashboard, select the ellipse (…) in the top right corner and
--         select View Chart to enter edit mode.

-- 3.16.5  Under data, click on the drop down arrow next to the X-Axis field
...
-- 3.16.6  Now let’s practice making another chart. Click Return to Member
--         Charts at the top, click the home button, click the Worksheets tab,
--         then select the Points for Members worksheet.

-- 3.16.7  Select the Chart and create a bar chart with Age as the X-Axis and
--         Member_avg as the bars. Make sure the labels are correct.

-- 3.16.8  Click on the Worksheet name drop down arrow and select Move to ->
--         Member Charts in the dashboards.

-- 3.16.9  Click Return to Member Charts at the top of the screen.

-- 3.16.10 Move the two charts by selecting one and dragging it where you would
--         like it to be.

-- 3.17.0  Working with the Data section
...
-- 3.17.2  In the left hand navigation bar, select the Data->Databases option
--         from the menu.
