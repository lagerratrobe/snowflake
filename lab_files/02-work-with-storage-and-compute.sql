...
-- 2.2.2   Locate and click VIPER_DB in the database list.
...
-- 2.2.11  Create a schema in your database, and name it VIPER_SCHEMA.
...
CREATE SCHEMA VIPER_DB.VIPER_SCHEMA;
...
-- 2.5.2   Using SQL, create a warehouse named VIPER_WH2, with the following
--         characteristics.
...
-- 2.5.15  Locate VIPER_WH2 and verify the Status is now Started.
...
-- 2.5.17  Suspend VIPER_WH2:

ALTER WAREHOUSE VIPER_WH2 SUSPEND;
...
-- 2.5.19  Alter VIPER_WH2 and set the WAREHOUSE_SIZE to XSmall:

ALTER WAREHOUSE VIPER_WH2 SET WAREHOUSE_SIZE=XSmall;


-- 2.5.20  Suspend and then drop VIPER_WH2:
...
--         VIPER_WH2 cannot be suspended.
--         Proceed with the statement below to drop the warehouse:

DROP WAREHOUSE VIPER_WH2;


-- 2.5.21  Show warehouses to verify that VIPER_WH is still listed, but
--         VIPER_WH2 is not.
