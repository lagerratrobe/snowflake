...
-- 4.0.0   Explore Snowflake Caching
--         Expect this lab to take approximately 45 minutes.

-- 4.1.0   Metadata Caching
...
-- 4.1.5   Suspend your warehouse. Be aware you will get an error if your
...
-- 4.1.7   Click the Query ID at the top of the result pane, then click the link
--         to open the profile.

-- 4.1.8   Click the profile tab. It should show that 100% of the result came
--         from metadata cache:
...
-- 4.2.0   Data Caching in the Compute Cluster

-- 4.2.1   Disable USE_CACHED_RESULT so you are only using metadata cache and
--         the data cache (not the query result cache):

ALTER SESSION SET USE_CACHED_RESULT = FALSE;


-- 4.2.2   Run Query 1 of the TPCH benchmark. This will automatically resume
--         your warehouse:
...
-- 4.2.3   Review the Query Profile and view the metric "Percentage Scanned from
--         Cache."
...
-- 4.2.6   Run the following query which is similar, but JOINs the data to
--         another table:
...
-- 4.3.0   Explore Query Result Caching
...
SELECT l_returnflag, l_linestatus,
    SUM(l_quantity) AS VIPER_sum_qty,
    SUM(l_extendedprice) AS sum_base_price,
...
-- 4.3.9   Bring up the Query Profile and check it. The query completed without
--         using a warehouse, because the results were accessible in the query
--         result cache.

-- 4.3.10  Grant privileges to your warehouse to the role SYSADMIN, and then
--         change to that role:

GRANT USAGE ON WAREHOUSE VIPER_WH TO ROLE SYSADMIN;
USE ROLE SYSADMIN;
...
-- 4.3.12  Check the query profile. Did the new role use any cache? Why or why
--         not?
...
-- 4.3.14  Set your context to TRAINING_ROLE, SNOWFLAKE_SAMPLE_DATA, TPCH_SF100,
--         and VIPER_WH:
...
-- 4.3.17  Change the line AS VIPER_sum_qty to AS VIPER_sum_qty_new and run
--         the query again. What do you think will happen?

SELECT l_returnflag, l_linestatus,
    SUM(l_quantity) AS VIPER_sum_qty_new,
    SUM(l_extendedprice) AS sum_base_price,
    SUM(l_extendedprice * (l_discount)) AS sum_disc_price,
    SUM(l_extendedprice * (l_discount) * (1+l_tax)) AS sum_charge,
    AVG(l_quantity) AS avg_qty,
