...
-- 8.0.0   Explore Semi-Structured JSON Data
...
-- 8.1.5   Select all columns from the isd_2019_daily Table, limit the results
...
-- 8.1.6   In the Results set, click on the JSON data to pull up the Details
--         pane with the VARIANT data.
--         This will display the structure of a single record of the JSON data.

-- 8.1.7   Use FLATTEN to extract the top-level keys from the V column. Use
--         LIMIT 10:
...
-- 8.1.8   Run a query to extract the time, station, country, elevation, temp,
...
-- 8.1.9   Run the following to display the types of columns from the last run
--         query:
...
-- 8.1.10  Return to the previous instance as that data pertains more to what we
...
-- 8.1.12  Remove the double quotes around the values by casting each value as a
...
-- 8.1.13  Run the following to display the types of columns from the last run
--         query:
...
-- 8.2.0   Extract and Transform the Data

-- 8.2.1   Extract the weather station reported air temperature data. Instead of
...
-- 8.2.3   Extract the weather station reported air temperature data that has
...
-- 8.2.4   Limit the results to records for a 7 day period starting on
...
-- 8.2.5   Write a query to pull country, station name, and highest temperatures
...
-- 8.2.6   Run the same query, but return the temperature in Fahrenheit instead
--         of Celsius.
...
-- 8.2.7   Run the following command, then click on one of the values in the V
...
-- 8.2.8   Run a query to return 10 rows showing the timestamp, the station
...
-- 8.2.9   Click on a row in the data column, to see the structure of the data
--         VARIANT. What additional information can you extract from the JSON
--         data provided?

-- 8.2.10  Use the FLATTEN table function (with a LATERAL JOIN) to iterate
...
-- 8.2.11  Run a query to calculate the average, max and min air temperature in
--         Celsius and Fahrenheit for each weather station.

-- 8.2.12  Run a query to calculate the average, max and min air temperature in
