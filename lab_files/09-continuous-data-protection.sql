...
-- 9.0.0   Continuous Data Protection
...
-- 9.1.0   Use Zero-Copy Clone to Copy Database Objects
...
-- 9.1.5   Create a copy of the LINEITEM table that is found in the
...
-- 9.1.11  Run the same statement against LINEITEM_CLONE, to see the clone has
--         not changed:
...
-- 9.1.13  Select the new rows from the LINEITEM_CLONE table. Run the same
...
-- 9.1.14  Run the same SELECT against the original LINEITEM table, and verify
...
-- 9.1.15  Run a JOIN against the LIEITEM_COPY and LINEITEM_CLONE tables to see
...
-- 9.2.3   Display the table history and review the values in the dropped_on
...
-- 9.2.6   Rerun the table history and review the values in the dropped_on
--         column:
...
-- 9.3.0   Recover a Table to a Time Before a Change Was Made

-- 9.3.1   Rerun the table history and review the values in the dropped_on
...
-- 9.3.3   Query the data in the TESTDROP table to confirm that the four rows
--         have been inserted:
...
-- 9.3.5   Review the data in the TESTDROP table to confirm that only the values
--         1000 and 4000 remain:

SELECT * FROM testdrop;


-- 9.3.6   Navigate to the History tab and locate the query where you deleted
...
-- 9.3.8   Create a new table that holds the value of TESTDROP before the query
...
-- 9.3.9   Review the data in the restored table to confirm that the values 2000
--         and 3000 have been restored and you have four (4) records:

SELECT * FROM testdrop_restored;


-- 9.3.10  Execute a JOIN that shows rows that are in TESTDROP_RESTORED, but not
--         in TESTDROP:
...
-- 9.3.12  Query the Table. Notice the error message about the table not
--         existing:

SELECT * FROM testdrop;


-- 9.4.0   Object-Naming Constraints
...
-- 9.4.3   Query the data in the LoadData1 table to confirm that the four (4)
--         rows have been inserted:

SELECT * FROM loaddata1;


-- 9.4.4   Drop the LoadData1 Table and confirm you receive an error that the
...
-- 9.4.5   Create a new iteration of the LoadData1 table with the same structure
...
-- 9.4.6   Drop the LoadData1 table again and create a third iteration of the
...
-- 9.4.8   Rerun the table history and note that there are now multiple entries
...
-- 9.4.9   Rename the current LoadData1 table iteration to LoadData3 and then
...
-- 9.4.10  Select the data from LoadData1 and note that iteration 2 has been
--         restored:

SELECT * FROM loaddata1;


-- 9.4.11  Re-run the table history to verify that the second iteration of the
--         table is now active:

SHOW TABLES HISTORY;


-- 9.4.12  Rename the current LoadData1 table iteration to LoadData2, and undrop
--         loaddata1:
...
-- 9.4.13  Select the data from LoadData. You will see that the first iteration
--         has been restored:

SELECT * FROM loaddata1;


-- 9.4.14  Re-run the table history to verify that all iterations of the table
--         are now active:

SHOW TABLES HISTORY;


-- 9.4.15  Query each of the LoadData tables and verify the different
...
-- 9.4.16  Rerun the table history to verify that all iterations of the table
--         are now active:
